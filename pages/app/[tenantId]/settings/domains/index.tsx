...
const CustomDomains = () => {
    ...
    const { data, mutate } = useGet(
        router?.query?.tenantId && `/api/${router?.query?.tenantId}/domains`
    );
    const {
        register,
        handleSubmit,
    ...
    const deleteDomain = async (id: string) => {
        await deleteEntity({
            url: `/api/${router?.query?.tenantId}/domains/${id}`,
        });
        await mutate();
    };
        ...
        <>
            <div className="grid grid-cols-1 md:grid-cols-2">
                ...
                <div className="flex items-center">
                    <Link
                        href={`/app/${router?.query?.tenantId}/settings/domains/create`}
                    >
                        <button
                            type="button"
                                        ...
                                        {data &&
                                            ...
                                            data?.map((domain) => (
                                                <tr>
                                                    ...
                                                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                        <Link
                                                            href={`/app/${router?.query?.tenantId}/settings/domains/${domain.id}/edit`}
                                                            passHref
                                                            legacyBehavior
                                                        >
