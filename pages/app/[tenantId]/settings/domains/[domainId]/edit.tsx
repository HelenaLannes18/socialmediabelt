...
const EditLink = () => {
    ...
    const { data } = useGet(
        ...
        `/api/${router?.query?.tenantId}/domains/${router?.query?.domainId}`
    );
    ...
    const submit: SubmitHandler<CustomDomainForm> = async (inputs) => {
        const data = await patch({
            url: `/api/${router?.query?.tenantId}/domains/${router?.query?.domainId}`,
            data: inputs,
        });
    ...
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-2">
                <div>
                    <Heading1>Gerenciador de Domínios</Heading1>
                ...
                <div className="p-4 bg-gray-100 border-t-2 border-indigo-400 rounded-lg bg-opacity-5">
                    <div className="max-w-sm mx-auto md:w-full md:mx-0">
                        <div className="inline-flex items-center space-x-4">
                            <Heading2>Editar Dominio</Heading2>
                        </div>
                    </div>
                </div>
                <div className="space-y-6 bg-white">
                    <div className="items-center w-full p-4 space-y-4 text-gray-500 md:inline-flex md:space-y-0">
                        <h2 className="max-w-sm mx-auto md:w-1/3">Dominio</h2>
